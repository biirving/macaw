    ...
    def __init__(self, size: int, obs_dim: int, action_dim: int, discount_factor: float = 0.99,
        ...
        if stream_to_disk:
            ...
            if os.path.exists(path):
                ...
                self._terminals = np.memmap(f'{path}/terminals.array', mode='r', shape=(size, 1), dtype=np.bool_)
                ...
                self._terminals = np.memmap(f'{path}/terminals.array', mode='w+', shape=(size, 1), dtype=np.bool_)
